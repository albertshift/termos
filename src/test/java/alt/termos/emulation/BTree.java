...
public final class BTree<K, V, E extends Tree.Entry<K, V>> implements Tree<K, V, E> {
	...
	public BTree(Class<E> entryClass, int branchingFactor, final Comparator<? super K> keyComparator) {
	...
	public final class InnerNode extends Node {
		...
		@Override
		public void join(E entry, Node greater) {
			if (greater instanceof BTree.InnerNode) {
		...
		public Object put(E entry) {
			...
			if (result instanceof BTree.Split) {
		...
		protected boolean canJoin(Node lesserChild, Node greaterChild) {
			if (lesserChild instanceof BTree.LeafNode) {
	...
	public final class LeafNode extends Node {
		...
		@Override
		protected E rotateClockwise(E splitEntry, Node greater) {
			if (greater instanceof BTree.LeafNode) {
	...
	private void afterRemoveProcessing() {
		if (root instanceof BTree.InnerNode && root.length == 0) {
	...
	private boolean hasTreeEntries() {
		if (root instanceof BTree.LeafNode && root.length == 0) {
			return false;
		}
		return true;
	}
