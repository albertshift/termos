...
public class SimpleBTreePerformanceTests {
	...
	private void run(ValueFunction func) throws InterruptedException {	
		...
		SimpleBTree<Integer, Integer> btree = new SimpleBTree<Integer, Integer>(Integer.class, Integer.class, 1000, Comparators.INTEGER);
        
		t0 = System.currentTimeMillis();
		for (int i = 0; i != N; ++i) {
			btree.put(i, func.apply(i));
		}
		td = System.currentTimeMillis() - t0;
		...
		t0 = System.currentTimeMillis();
		for (int i = 0; i != N; ++i) {
			Assert.assertEquals(func.apply(i), btree.get(i));
		}
		td = System.currentTimeMillis() - t0;
